...
parseCommand schema "Add" rest = do restok <- (parseBySchema schema rest)
                                    Just (Add restok)
parseCommand schema "Get" value = case value of
                                       "" => Just (Get Nothing)
                                       val => case all isDigit (unpack val) of
                                                        False => Nothing 
...
sizeCounter : String -> Nat -> DataStore -> Maybe (String, DataStore)
sizeCounter acc k store@(MkData schema size items) = if k == size then Just (acc, store)
                                                            else let (Just (toadd, st)) = getEntry (cast k) store in
                                                                 sizeCounter (acc ++ show k ++ ": " ++ toadd) (S k) store
            

getWholeStore : (store : DataStore) -> Maybe (String, DataStore)
getWholeStore store@(MkData schema size stor)  = sizeCounter "" Z store
...
processInput store inp 
  = case parse (schema store) inp of
         ...
         Just (Get pos) => case pos of 
                                       Just num => getEntry num store
                                       Nothing => getWholeStore store
         Just Quit => Nothing
